...
const DRAG_ACTIVATION_TRESHOLD = 200
const BLOCK_TRANSITION_DURATION = 300
const ACTIVE_BLOCK_CENTERING_DURATION = 200
const DOUBLETAP_TRESHOLD = 150
const NULL_FN = () => {}
...
class SortableGrid extends Component {
  ...
  renderEmptyBlocks () {
    ...
    for (let index = this.items.length - 1; index < 4; index += 1) {
      const isFirst = index === 0
      const [{ width, height, ...blockStyle }] = this._getBlockStyle(index, false)
      const nextRow = index >= 2
      const x = nextRow ? index - 2 : index
      const style = {
        ...blockStyle,
        width,
        height,
        left: 176 + (x * width),
        top: nextRow ? height : 0,
        position: 'absolute',
      ...
      emptyBlocks.push(
        ...
        >
          <View style={[styles.block, styles.addPhotoButton]} />
        </Block>
      )
    }
        ...
        >
          {this.state.gridLayout &&
            this.items.map((item, index) => {
              const isFirst = parseInt(this.itemOrder.find(f => f.order === 0).key) === parseInt(index)
              // console.log({ item })
              return (
                <Block
                  key={`image-${index}`}
                  isFirst={isFirst}
                  style={this._getBlockStyle(index, isFirst)}
                  onLayout={this.saveBlockPositions(index)}
                  panHandlers={this._panResponder.panHandlers}
                  delayLongPress={this.dragActivationTreshold}
                  onLongPress={this.activateDrag(index)}
                  onPress={this.handleTap(item.props)}
                  itemWrapperStyle={this._getItemWrapperStyle(index, isFirst)}
  ...
  _defaultDragActivationWiggle = () => {
    if (!this.dragStartAnimation) {
      this.state.startDragWiggle.setValue(5)
  ...
  _getBlockStyle = (key, isFirst) => {
    // console.log({ key, state: { ...this.state } })
    // console.log('_getBlock', { key, isFirst })

    let secondStyle = null

    try {
      if (this._blockPositionsSet() && (this.initialDragDone || this.state.deleteModeOn)) {
        secondStyle = {
          position: 'absolute',
