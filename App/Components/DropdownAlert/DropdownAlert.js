...
export default class DropdownAlert extends Component {
  ...
  alert = params => {
    const { type } = params
    // if (validateType(type) === false) { return; }
    // if (typeof title !== 'string') title = title.toString()
    // if (typeof message !== 'string') message = message.toString()
    if (type !== 'message' && type !== 'match' && type !== 'info' && type !== 'error') return false
  ...
  close = action => {
    if (action === undefined) action = 'programmatic'
    var onClose = this.props.onClose
    if (action === 'cancel') onClose = this.props.onCancel
    if (this.state.isOpen) {
      if (this._closeTimeoutId != null) clearTimeout(this._closeTimeoutId)
      this.animate(0)
      setTimeout(() => {
        if (this.state.isOpen) {
  ...
  renderImage (source) {
    if (this.props.renderImage) return this.props.renderImage(this.state)
  ...
  renderTitle () {
    if (this.props.renderTitle) {
      return this.props.renderTitle(this.state)
    } else {
      return (
        <Label
  ...
  renderMessage () {
    if (this.props.renderMessage) {
      return this.props.renderMessage(this.state)
    } else {
      return (
        <Label
  ...
  onTap = () => {
    if (this.props.onTap) this.props.onTap(this.state)
  }
      ...
      return (
        ...
        >
          <TouchableOpacity
            ...
            onPress={!this.props.tapToCloseEnabled ? this.onTap : () => this.close('tap')}
            disabled={!this.props.tapToCloseEnabled && !this.props.onTap}
